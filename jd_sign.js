...
function setupCookie() {
  ...
  js_content = js_content.replace(/var Key = ''/, `var Key = '${cookies[0]}'`)
  if (cookies.length > 1) {
    js_content = js_content.replace(/var DualKey = ''/, `var DualKey = '${cookies[1]}'`)
  }
  if (cookies.length > 2) {
    let others = []
    for (let i = 2; i < cookies.length; i++) {
      others.push({ "cookie": cookies[i] })
    }
    let other_key = JSON.stringify(others)
    js_content = js_content.replace(/var OtherKey = ''/, `var OtherKey = '${other_key}'`)
  }
...
function sendNotificationIfNeed() {
  ...
  const options = {
    ...
    body: { "msgtype": "text", "text": { "content": desp } },
    json: true,
    method: 'POST'
  }
