...
// 京东脚本文件
const js_url = 'https://raw.fastgit.org/NobyDa/Script/master/JD-DailyBonus/JD_DailyBonus.js'
// 下载脚本路劲
...
function setupCookie() {
    ...
    let cookie_str_array = cookie.split("|")
    let cookie_json_array = []
    for (let i = 0; i < cookie_str_array.length; i++) {
        cookie_json_array.push({"cookie": cookie_str_array[i]})
    }
    js_content = js_content.replace(/var OtherKey = ``/, `var OtherKey = \`${JSON.stringify(cookie_json_array)}\``)
...
async function main() {
    ...
    download(js_url, './').then(res => {
        ...
        fs.writeFileSync(error_path, JSON.stringify(err), 'utf8')
    })
}
